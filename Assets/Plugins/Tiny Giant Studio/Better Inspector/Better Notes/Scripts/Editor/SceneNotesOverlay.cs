...
namespace TinyGiantStudio.BetterInspector
{
    ...
    {
        static BetterInspectorEditorSettings commonEditorSettings;
        static NoteSettings noteSettings;
        static SceneNotesManager notesManager;
        static GUIStyle noteStyle;

        // ReSharper disable once FieldCanBeMadeReadOnly.Local
        static Dictionary<Transform, NoteWithBound> visibleNotes = new();
        static float lastUpdatedOn;
        ...
        static SceneNotesOverlay()
        {
            EditorApplication.delayCall += () =>
            {
                ...
                if (commonEditorSettings == null) commonEditorSettings = BetterInspectorEditorSettings.instance;
                if (commonEditorSettings == null) return;
                commonEditorSettings.Register("Open Note Editor", () => NoteEditor.ShowEditor());

                NotesBetterTransformIntegration notesBetterTransformIntegration =
                    ScriptableObject.CreateInstance<NotesBetterTransformIntegration>();
                commonEditorSettings.BetterTransformSelectionUpdate +=
        ...
        static void OnSceneGUI(SceneView sceneView)
        {
            ...
            UpdateCache();
            foreach (KeyValuePair<Transform, NoteWithBound> note in visibleNotes.Where(note => note.Key != null))
            {
        ...
        {
            if (lastUpdatedOn > Time.realtimeSinceStartup)
                return;

            //Debug.Log("Updated cache");

            lastUpdatedOn = Time.realtimeSinceStartup + Random.Range(0.25f, 2f);

            visibleNotes.Clear();
            foreach (KeyValuePair<Transform, Note> note in notesManager.Notes)
            {
                ...
                visibleNotes.Add(note.Key, new(note.Value, bounds));
            }
        }
        ...
        static bool ShouldDraw(SceneView sceneView)
        {
            ...
            if (noteSettings == null) noteSettings = NoteSettings.instance;
            if (noteSettings == null) return false;
            if (!noteSettings.showNotes) return false;
            if (!noteSettings.showNotesGizmo) return false;
            if (Application.isPlaying && !noteSettings.showNotesGizmoDuringPlayMode) return false;

            if (notesManager == null) notesManager = SceneNotesManager.Instance;
            return notesManager != null;
        }
        ...
        static void UpdateStyle()
        {
            ...
            noteStyle = new(EditorStyles.largeLabel)
            {
            ...
            Color backgroundColor, Vector2 padding = default)
        {
            ...
            Vector2 textSize = noteStyle.CalcSize(content);
            ...
            noteStyle.normal.textColor = textColor;
            GUI.Label(
