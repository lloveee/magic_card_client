...
namespace TinyGiantStudio.BetterInspector
{
    public class NotesBetterTransformIntegration : Editor
    {
        ...
        void UpdateGizmoUI(VisualElement root, Transform targetTransform)
        {
            ...
            {
                notesGizmoVisualTreeAsset =
                    AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(NotesGizmoVisualTreeAssetFileLocation);
                if (notesGizmoVisualTreeAsset == null)
                {
                    string assetPath = AssetDatabase.GUIDToAssetPath(NotesGizmoVisualTreeAssetGuid);
                    if (!string.IsNullOrEmpty(assetPath))
                        notesGizmoVisualTreeAsset = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(assetPath);
                }
            }

            // Debug.Log(AssetDatabase.GUIDFromAssetPath(NotesGizmoVisualTreeAssetFileLocation));
        ...
        void CreateNoteTooltipUI(VisualElement root, Note note)
        {
            ...
            {
                tooltipVisualTreeAsset =
                    AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(TooltipVisualTreeAssetFileLocation);
                if (tooltipVisualTreeAsset == null)
                {
                    string assetPath = AssetDatabase.GUIDToAssetPath(TooltipVisualTreeAssetGuid);
                    if (!string.IsNullOrEmpty(assetPath))
                        tooltipVisualTreeAsset = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(assetPath);
                }
            }

            // Debug.Log(AssetDatabase.GUIDFromAssetPath(TooltipVisualTreeAssetFileLocation));
