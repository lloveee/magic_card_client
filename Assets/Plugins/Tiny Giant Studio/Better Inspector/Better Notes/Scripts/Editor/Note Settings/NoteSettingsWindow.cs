...
namespace TinyGiantStudio.BetterInspector
{
    public class NoteSettingsWindow : EditorWindow
    {
        ...
        void UpdateSceneNotes()
        {
            ...
            {
                notesListItemVisualTreeAsset =
                    AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(NotesListItemVisualTreeAssetFileLocation);
                if (notesListItemVisualTreeAsset == null)
                {
                    string assetPath = AssetDatabase.GUIDToAssetPath(NotesListItemVisualTreeAssetGuid);
                    if (!string.IsNullOrEmpty(assetPath))
                        notesListItemVisualTreeAsset = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>(assetPath);
                }
            }
            // Debug.Log(AssetDatabase.GUIDFromAssetPath(NotesListItemVisualTreeAssetFileLocation));
