...
namespace CoreDomain.Scripts.Services.SceneInitiatorService
{
    public class SceneInitiatorsService : ISceneInitiatorsService
    {
        ...
        public async Awaitable InvokeInitiatorLoadEntryPoint(SceneType sceneType, IInitiatorEnterData enterData,
            CancellationTokenSource cancellationTokenSource)
        {
            await _sceneInitiators[sceneType].LoadEntryPoint(enterData, cancellationTokenSource);
        }

        public async Awaitable InvokeInitiatorStartEntryPoint(SceneType sceneType, IInitiatorEnterData enterData,
            CancellationTokenSource cancellationTokenSource)
        {
            await _sceneInitiators[sceneType].StartEntryPoint(enterData, cancellationTokenSource);
        }

        public async Awaitable InvokeInitiatorExitEntryPoint(SceneType sceneType, CancellationTokenSource cancellationTokenSource)
        {
            await _sceneInitiators[sceneType].ExitEntryPoint(cancellationTokenSource);
        }
    }
}